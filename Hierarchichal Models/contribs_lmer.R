...
get_lme4_str_formulas <- function(m_h_lm) {
  ...
  out <- list(fe_str_f = fe_str_f,
              re_str_f = re_str_f)
  
  return(out)
}

calc_contribs <- function(m_h_lm, base) {
  ...
  #         str_remove("\\(|\\)") %>% 
  #         str_squish)
  str_ff <- get_lme4_str_formulas(m_h_lm)
  ...
  contrib_f <- 
    as.formula(paste(str_response, "~ ", paste(model_vars, collapse = "+")))
  
  mm <- model.matrix(contrib_f, base)
  mm <- mm[, order(colnames(mm))]
  
  progs <- unique(base$name_edit)
  ...
  for (prog in progs) {
    ...
    idx_prog <- which(base$name_edit == prog)
